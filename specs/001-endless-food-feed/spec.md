...
- **FR-006**: Players MUST be able to perform an engagement action on a post, visible on the post itself without discovery, explanation, or a secondary screen. The action is a **single binary gesture** — one tap, on and off — not a choice between distinct responses (decision 0007). Session state is therefore a set of post identities, not a map of responses.
...
- **FR-010**: At least one engagement action MUST produce a change in the feed that is visible to the player within a small number of subsequent posts. The change is **more posts carrying a tag of the post engaged with** (decision 0007) — legible to a playtester, and an honest miniature of the real mechanic without pre-empting the escalation curve, which is phase 2's. Subject to the precedence recorded in decision 0009: the 40% off-affinity floor outranks this guarantee where the two compete.
